...
echo 3 Installs
echo 4 First Aid
echo 5 OS Fresh Reload
echo 6 New Computer
echo 7 Checkover
echo 8 Update the TTracker
echo 9 Update Virophage and Reload
...
if %choice% == 5 goto fresh.reload
if %choice% == 6 goto new.computer
if %choice% == 7 goto checkover
if %choice% == 8 goto up
if %choice% == 9 goto update.reload
...
:update.reload
if exist "c:\acr\var\alpha.var" "%Virophage.Source%Setup.bat"
...
echo.
echo.
echo 1 Avast Professional
echo 2 Malware Bytes Registered
echo 3 Wot
echo 4 Crashplan
echo 5 MSE
echo 6 Avira Registered
echo 7 Go Back
set /p install="Enter Choice: "
if %install% == 1 goto install.avast.paid
if %install% == 2 goto installmbamreg
if %install% == 3 goto installwot
if %install% == 4 goto install.crashplan
if %install% == 5 goto install.mse
if %install% == 6 goto installavirareg
if %install% == 7 goto main.menu

:install.avast.paid
start "" "c:\acr\app\avast_pro_antivirus_setup.exe"


:install.mse
echo install.me  >%virvar%resume.var
echo There is currently only support for Windows 7.  If you have another operating system this will not work, but please let Shard know so he can set it up using this computer!
for /f "delims=" %%x in (c:\acr\var\source.var) do set "source=%%x"
for /f "delims=" %%x in (c:\acr\var\winvernum.var) do set "ver=%%x"
set ver=%ver: =%
...
for /f "delims=" %%x in (%virvar%architecture.var) do set "architecture=%%x"
if %architecture% == AMD64 goto crashplan.64
goto crashplan.32

:crashplan.64
start %virapp%crashplan64.exe
goto main.menu

:crashplan.32
start %virapp%crashplan.exe
goto main.menu
...
if %choice% == 6 goto safe.mode.combofix
if %choice% == 7 goto safe.mode.mbam
...
if %choice% == 15 goto sninite
if %choice% == 16 goto safe.mode.sfc
if %choice% == 17 goto smsconfig
cls
echo ========================================
echo [%autorunstate%] Virophage Will Start Next Boot
echo COMPUTERNAME: %computername%
...
if exist "c:\acr\app\tfc.exe" goto tfc.exist
...
rem	echo Firewall disable because log says firewall was alreadry off.
rem	)
...
if exist "C:\Program Files\Malwarebytes' Anti-Malware\mbam.exe" goto safe.mode.skip.mbam.install
...
start /wait %virdir%app\sfirstupdates.exe
start /wait %virbat%cleanup.firstaid.bat
echo Did %name% finish completely?
echo Please initial
set /p initial=
echo (%initial%) %name%  (DATE: %date% %time:~0,-6% USER:"%username%" %windows.mode%) >>%virlog%
...
echo 3 TFC
echo 4 ADWCleaner
echo 4.5 Junk Removal Tool
echo 5 Reset Internet Settings
echo 6 Reset Firewall Settings
echo 7 Combofix
echo 8 Malware Bytes
echo 9 SAS Portable
echo 10 Spybot
echo 11 Secondary AV Scan
echo 12 Hitman Pro
echo 14 TDSSKiller
echo 15 AutoRuns
echo 16 HijackThis!
...
echo 24 WOT
echo 25 AdBlockers
echo 26 Firefox Proxy Settings
echo 27 Arbiter Virus?
echo.
...
if %choice% == 4 goto normal.mode.adwcleaner
if %choice% == 4.5 goto normal.mode.JRT
...
if %choice% == 22 goto normal.mode.ninite
if %choice% == 23 goto nregistry
if %choice% == 24 goto normal.mode.wot
if %choice% == 25 goto normal.mode.adblockers
if %choice% == 26 goto firefox.proxy
if %choice% == 27 goto arbiter.virus
...
:normal.mode.combofix
echo normal.mode.combofix >%virvar%resume.var
echo COMBOFIX
color 04
start /wait "ComboFix" "c:\acr\bat\combofix.bat.lnk"
color 4f
...
:normal.mode.ninite
echo normal.mode.ninite >%virvar%resume.var
echo NINITE UPDATES
set name=Ninite Updates
%virdir%app\nfirstupdates.exe
color 04
...
rem echo.
rem echo OPENING CHROME FOR MALWARE BYTES LICENSING
rem ECHO.
...
rem start /min %virbat%mbamchrome.bat
...
echo Webcam?
start chrome.exe -incognito --new-window "http://www.allthingsinc.com/tek/test-webcam/"
set name=Webcam
echo Initial
set /p initial=
echo (%initial%) %name%(DATE: %date% %time:~0,-6% USER:"%username%" %windows.mode%) >>%virlog%
